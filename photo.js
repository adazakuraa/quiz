...
let birds=[];
let sub=["キジ","カモ","カイツブリ","ネッタイチョウ","サケイ","ハト","アビ","ミズナギドリ","コウノトリ","カツオドリ","ペリカン","ツル","カッコウ","ヨタカ","アマツバメ","チドリ","タカ","フクロウ","サイチョウ","ブッポウソウ","キツツキ","ハヤブサ","スズメ","ダチョウ","ヒクイドリ","エボシドリ","オウム","フラミンゴ","ジャノメドリ","キヌバネドリ","キーウィ","ネズミドリ","ノガンモドキ","ノガン","ペンギン","レア","シギダチョウ","ツメバケイ"]

function create_photo(){
    var photo = document.getElementById("lightgallery");
    let count=2;
    for (var n=0 ;n<pic_bird.length;n++){
        u=pic_bird[n].split('（')[0];
        
        let bird={name:u,cate:pic_bird[n].split('（')[1].split('）')[0]};
        birds.push(bird);
        ...
        if(s!=pic_bird[n].split('（')[1].split('）')[0]){
            s=pic_bird[n].split('（')[1].split('）')[0]
            var p = document.createElement('p');
            var hr =document.createElement('hr');
